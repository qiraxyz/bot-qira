...
async function starts() {
	const qiraxyz = new WAConnection()
	qiraxyz.logger.level = 'warn'
	qiraxyz.on('qr', () => {
		console.log(color('[qira]','aqua'), color("Scan QR code for Connecting...", "yellow"))
	})
	fs.existsSync('./session/qira.json') && qiraxyz.loadAuthInfo('./session/qira.json')
qiraxyz.on('connecting', () => {
        const time_connecting = moment.tz('Asia/Jakarta').format('HH:mm:ss')
        console.log(color('[qira]','aqua'), color("Wait For Connecting...", "yellow"))
    })
qiraxyz.on('open', () => {
        const time_connect = moment.tz('Asia/Jakarta').format('HH:mm:ss')
        console.log(color('[qira]','aqua'), color(`Done Connecting`, "aqua"))
        start('')
    })
	await qiraxyz.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./session/qira.json', JSON.stringify(qiraxyz.base64EncodedAuthInfo(), null, '\t'))

qiraxyz.on('group-participants-update', async (anu) => {
		if (!_welcom.includes(anu.jid)) return
		try {
			const mdata = await qiraxyz.groupMetadata(anu.jid)
			if (anu.action == 'add') {
...
Ketik ${prefix}verify untuk memulai menggunakan bot.`
				qiraxyz.sendMessage(mdata.id, hai, MessageType.text, { contextInfo: {"mentionedJid": [num]}})
			} else if (anu.action == 'remove') {
				num = anu.participants[0]
				bye = `Selamat tinggal @${num.split('@')[0]}👋`
				qiraxyz.sendMessage(mdata.id, bye, MessageType.text, {contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
...
qiraxyz.on('chat-update', async (mek) => {
...
const totalchat = await qiraxyz.chats.all()
const botNumber = qiraxyz.user.jid
			
//-- Group Metadata
  const isGroup = from.endsWith('@g.us')
  const sender = isGroup ? mek.participant : mek.key.remoteJid
  const groupMetadata = isGroup ? await qiraxyz.groupMetadata(from) : ''
  ...
  const pushname = qiraxyz.contacts[sender] != undefined ? qiraxyz.contacts[sender].vname || qiraxyz.contacts[sender].notify: undefined

//-- other
  const isUrl = (url) => {
			    return url.match(new RegExp(/https?:\/\/(www\.)?[-a-zA-Z0-9@:%._+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_+.~#?&/=]*)/, 'gi'))
			}
  const { wa_version, mcc, mnc, os_version, device_manufacturer, device_model } = qiraxyz.user.phone

//-- function reply
			const reply = (teks) => {
				qiraxyz.sendMessage(from, teks, text, {quoted:mek })
			}
			
			const sendMess = (hehe, teks) => {
				qiraxyz.sendMessage(hehe, teks, text)
			}
			
			const mentions = (teks, memberr, id) => {
				(id == null || id == undefined || id == false) ? qiraxyz.sendMessage(from, teks.trim(), extendedText, {contextInfo: {"mentionedJid": memberr}}) : qiraxyz.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": memberr}})
			}
      const replyimg = (pesan, tipe, rep1, rep2) => {
        qiraxyz.sendMessage(from, pesan, tipe, { quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: {
"imageMessage": {
"mimetype": "image/jpeg",
"caption": `${rep1}`,
"fileLength": "201809",
...
//-- watermark stiker
			function addMetadata(packname, author) {	
				if (!packname) packname = 'qira'; if (!author) author = pushname ;	
...
//--Console log grup
if (!isGroup && isCmd) console.log(color('[qira]','aqua'), "CP", color(command, "orange"), "from", (sender.split('@')[0]),  args.length)
			
//--Console log chat pribadi
if (isGroup && isCmd) console.log(color('[qira]','aqua'), "GC", color(command, "orange"), "from", (sender.split('@')[0]), "in", (groupName), args.length)
...
if (isGroup) {
  ...
  qiraxyz.sendMessage(from, `Maaf syarat member harus di atas ${memberlimit}, selamat tinggal 👋🏻`, text)

    setTimeout(() => {
    qiraxyz.groupLeave(from) // ur cods
  }, 5000) // 1000 = 1s,
}
  } catch {
console.error(err)
...
const linkwa = 'https://chat.whatsapp.com/'
		...
		if (!isGroup) return
		if (!isAnti) return
    if (!isBotGroupAdmins) return reply('Untung qira bukan admin, kalo iya gua kick lu')
    linkgc = await qiraxyz.groupInviteCode (from)
    if (budy.includes(`${linkwa}${linkgc}`)) return reply('Untung Link group ini')
		if (isGroupAdmins) return reply(`Hmm mantap min`)
		qiraxyz.updatePresence(from, Presence.composing)
		var Kick = `${sender.split("@")[0]}@s.whatsapp.net`
		setTimeout( () => {
		reply('byee')
		}, 1100)
		setTimeout( () => {
		qiraxyz.groupRemove(from, [Kick]).catch((e) => {console.log(`*ERROR:* ${e}`)}) 
					}, 1000)
		setTimeout( () => {
...
if (isCmd) qiraxyz.chatRead(from)
...
case 'info':
  uptime = process.uptime()
  reply(info(qiraxyz, uptime, process, wa_version, mcc, mnc, os_version, device_manufacturer, device_model))
					break
...
//-- ganteng cek
case 'gantengcek':
  if (args.length < 1) return reply(`Contoh : ${prefix}gantengcek qira`)
  if (!isVerify) return reply(userB())
  random = `${Math.floor(Math.random() * 100)}`
  gan = value
  cek = `Target : *${gan}*
Persentase : ${random}%`
qiraxyz.sendMessage(from, cek, text, {quoted: mek})
break

//--- cantik cek
case 'cantikcek':
  if (args.length < 1) return reply(`Contoh : ${prefix}cantikcek qira`)
  if (!isVerify) return reply(userB())
  random = `${Math.floor(Math.random() * 100)}`
  can = value
  cek = `Target : *${can}*
...
case 'ocr':
  if (!isVerify) return reply(userB())
			...
			const media = await qiraxyz.downloadAndSaveMediaMessage(encmedia)
			reply(wait())
...
case 'tomp3':
					...
					media = await qiraxyz.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.mp4')
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('Gagal, pada saat mengkonversi video ke mp3')
						bufferlkj = fs.readFileSync(ran)
						qiraxyz.sendMessage(from, bufferlkj, audio, {mimetype: 'audio/mp4', quoted: mek})
						fs.unlinkSync(ran)
					})
					break
...
case 'slow':
  ...
  if (!isVerify) return reply(userB())
					...
					exec(`ffmpeg -i ${media} -filter:a "atempo=0.7,asetrate=44100" ${ran}`, (err, stderr, stdout) => {
						...
						qiraxyz.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:true, quoted: mek})
						fs.unlinkSync(ran)
					})
				break
...
case 'tovn':
  ...
  if (!isPrem) return reply(premi())
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						qiraxyz.sendMessage(from, topt, audio, {mimetype: 'audio/mp4', quoted: mek, ptt:true})
						})
					break
...
case 'nightcore':
  ...
  if (!isPrem) return reply(premi())
	if (!isQuotedAudio) return reply('Reply Audionya')
					...
					core = await qiraxyz.downloadAndSaveMediaMessage(night)
					ran = getRandom('.mp3')
					reply(wait())
					exec(`ffmpeg -i ${core} -filter:a atempo=1.06,asetrate=44100*1.25 ${ran}`, (err, stderr, stdout) => {
						...
						if (err) return reply('Error!')
						hah = fs.readFileSync(ran)
						qiraxyz.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:false, quoted: mek, ptt: true})
						fs.unlinkSync(ran)
					   })
				       break
...
case 'gets': case 'getstick': case 'getstik':
			  if (args.length < 1) return reply(`Masukan juga nama stiker pada ${prefix}liststik`)
				...
				qiraxyz.sendMessage(from, result, sticker, selepbot)
				} catch {
...
case 'addvn':
				...
				qiraxyz.sendMessage(from, `Sukses Menambahkan Audio\nCek dengan cara ${prefix}listvn`, MessageType.text, { quoted: mek })
...
case 'getvn':
			  if (args.length < 1) return reply(`Masukan juga nama vn pada ${prefix}listvn`)
				...
				qiraxyz.sendMessage(from, buffer, audio, { mimetype: 'audio/mp4', quoted: mek, ptt: true })
				} catch {
				  reply('Pack tidak terdaftar')
				}
...
//--- mengambil foto
			case 'getimg':
			  if (args.length < 1) return reply(`Masukan juga nama image pada ${prefix}listimg`)
				...
				qiraxyz.sendMessage(from, buffer, image, { quoted: mek })
				} catch {
				  reply('Pack tidak terdaftar')
				}
...
//--- mengambil video
			case 'getvid':
			  if (args.length < 1) return reply(`Masukan juga nama video pada ${prefix}listvid`)
				...
				qiraxyz.sendMessage(from, buffer, video, { mimetype: 'video/mp4', quoted: mek })
				} catch {
				  reply('Pack tidak terdaftar')
				}
  ...
  if (!isVerify) return reply(userB())
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								console.log('Finish')
								exec(`webpmux -set exif ${addMetadata('qira', pushname)} ${ran} -o ${ran}`, async (error) => {
									if (error) return reply(stick())
									qiraxyz.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
									fs.unlinkSync(media)	
									fs.unlinkSync(ran)	
								})
							})
...
case 'toimg':
  if (!isVerify) return reply(UserB())
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						qiraxyz.sendMessage(from, buffer, image, {quoted:mek, caption: 'Gini kan'})
						fs.unlinkSync(ran)
					})
					break
...
case 'clearchat':
		if (!isOwner) return reply(ownerB())
		anu = await qiraxyz.chats.all()
		list_chat = await qiraxyz.chats.all()
    for (let chat of list_chat) {
    qiraxyz.modifyChat(chat.jid, "delete")
    }
...
case 'blocklist':
  if (!isVerify) return reply(userB())
      teks = ` ≡ *BLOCKLIST*\nNomor di bawah adalah list nomor yang di Blokir karena melanggar aturan dan melakukan spam pada bot\n\n*Total* : ${blocked.length}\n┌───⊷ *LIST* ⊶\n`
	    ...
	    teks += `└──────────────`
	    qiraxyz.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": blocked}})
					break
...
case 'request':
  if (!isVerify) return reply(userB())
  yoi = value
  if (args.length < 1) return reply('Masukan fitur yang kamu inginkan ada di Bot qira')
...
case 'listonline':
  ...
  if (!isGroup) return reply(group())
			    ...
			    let online = [...Object.keys(qiraxyz.chats.get(ido).presences), qiraxyz.user.jid]
			    qiraxyz.sendMessage(from, 'List Online:\n' + online.map(v => '- @' + v.replace(/@.+/, '')).join`\n`, text, { quoted: mek,
...
//--- buka tutup grup
					...
					if (args[0] === 'open') {
						...
						qira.groupSettingChange(from, GroupSettingChange.messageSend, false)
					} else if (args[0] === 'close') {
						reply(`Menutup Group *${groupMetadata.subject}*`)
						qira.groupSettingChange(from, GroupSettingChange.messageSend, true)
					}
					break
...
case 'kick':
			    ...
			    if (!isPrem) return reply(premi())
					...
					if (mentioned.length > 1) {
						...
						qiraxyz.groupRemove(from, mentioned)
					} else {
...
case 'promote':
				  if (!isVerify) return reply(userB())
					...
					if (mentioned.length > 1) {
						...
						mentions(teks, mentioned, true)
						qiraxyz.groupMakeAdmin(from, mentioned)
					} else {
...
case 'demote':
			    if (!isVerify) return reply(userB())
					...
					if (mentioned.length > 1) {
						...
						mentions(teks, mentioned, true)
						qiraxyz.groupDemoteAdmin(from, mentioned)
					} else {
...
case 'linkgc':
  if (!isVerify) return reply(userB())
			  if (!isGroupAdmins && !isOwner) return reply(admin())
				...
				qiraxyz.sendMessage(from, yeh, text, {quoted: mek})
				break
...
case 'notif':
    ...
    gc = await qiraxyz.groupMetadata(from);
...
await qiraxyz.sendMessage(from, options, text)
break
...
case 'leave': 
				if (!isGroup) return reply(group())
				if (!isOwner) return reply(ownerB())
				anu = await qiraxyz.groupLeave(from, `Bye *${groupMetadata.subject}*`, groupId)
				break
...
case 'setppgc':
				...
				try {
				media = await qiraxyz.downloadAndSaveMediaMessage(mek)
				await qiraxyz.updateProfilePicture (from, media)
				reply(wait())
...
case 'clone':
		...
		if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Tag target yang ingin di clone')
					...
					try {
						pp = await qiraxyz.getProfilePicture(id)
						buffer = await getBuffer(pp)
						qiraxyz.updateProfilePicture(botNumber, buffer)
...
case 'totaluser':
  if (!isOwner) return reply(ownerB())
					...
					qiraxyz.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": _user}})
					break
...
case 'del':
					if (!isGroup)return reply(group())
					try {
					qiraxyz.deleteMessage(from, { id: mek.message.extendedTextMessage.contextInfo.stanzaId, remoteJid: from, fromMe: true })
					} catch (e) {
...
case 'daftar':
			...
			qiraxyz.sendMessage(from, capt, text, {quoted: mek})
					break
...
case 'wait':
  if (!isVerify) return reply(userB())
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						reply(wait())
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						media = await qiraxyz.downloadMediaMessage(encmedia)
						await _wait(media).then(res => {
							qiraxyz.sendMessage(from, res.video, video, {quoted: mek, caption: res.teks.trim()})
						}).catch(err => {
							reply(err)
						})
					} else {
...
case 'term':
  ...
  if (args.length < 1) return reply(termux())
	exec(cmd, (err, stdout) => {
	if(err) return qiraxyz.sendMessage(from, `*Root Term*\n ${err}`, text, { quoted: mek })
	if (stdout) {
	qiraxyz.sendMessage(from, stdout, text)
		}
	})
break
...
case 'return':
  if (!isOwner) return reply(ownerB())
			return qiraxyz.sendMessage(from, JSON.stringify(eval(args.join(' ')), null, '\n'), text, { quoted: mek })
		break
